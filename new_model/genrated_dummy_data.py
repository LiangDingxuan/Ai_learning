...
model = build_model(input_shape=X_train.shape[1], output_shape=y_train.shape[1], learning_rate=0.001, l1=1e-5, l2=1e-4, dropout_rate=0.12)
...
# Find the top 15 predictions in order
top_15_indices = np.argsort(predictions, axis=1)[0][::-1][:15]
top_15_searches = [f'P{str(i+1).zfill(2)}' for i in top_15_indices]

print("Top 15 Predicted Searches:", top_15_searches)
